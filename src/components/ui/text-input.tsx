...
export function TextInput({
  ...
  return (
    <div className="flex w-full flex-col gap-2">
      <label
        htmlFor={id}
        className="font-field text-xs font-bold leading-4 text-[var(--color-field-label)] max-sm:text-[11px]"
      >
        {label}
      </label>
      <div
      ...
      >
        {icon && (
          <Image
            ...
            width={20}
            height={20}
            className="size-5 shrink-0 max-sm:size-4"
          />
        )}
        <input
          id={id}
          ...
          className="min-w-0 flex-1 border-none bg-transparent font-field text-base leading-6 text-[var(--color-ink)] outline-none placeholder:text-zinc-400 max-md:text-[16px] max-sm:text-sm"
          {...props}
        />
      </div>
    </div>
